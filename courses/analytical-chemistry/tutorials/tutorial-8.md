...
**讨论：** 如果忘记过滤除去 AgCl 沉淀，SCN⁻ 会与 AgCl 反应转化为 AgSCN，导致消耗的 SCN⁻ 量增加，即 n(Ag⁺)过量 偏高，最终计算出的 Cl⁻ 浓度会偏低。

$$n(\text{Ag}^+)_\text{反应} \downarrow = n(\text{Ag}^+)_\text{总} - n(\text{Ag}^+)_\text{过量} \uparrow$$
